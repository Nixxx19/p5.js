...
|---|---|---|---|---|
| 1 | community bonding: study geometry apis, read PShapeOBJ.java, draft architecture doc, confirm sign-off with mentors | week 1-2 | 40h | 5h |
| 2 | extend `parseMtl()`: parse all mtl tokens, build texture loading pipeline | week 3-5 | 35h | 5h |
| 3 | rewrite `parseObj()` slicer: per-material vertex buckets, uv mapping per slice, face-index localisation | week 6-8 | 55h | 15h |
| 4 | extend `Renderer3D.model()`: multi-draw loop, per-slice material and texture binding, buffer cache per slice | week 9-11 | 50h | 15h |
...
| 8 | api parity audit, edge cases, performance profiling, file follow-up issues for unimplemented features | week 19-20 | 20h | 5h |
| **core total** | | **week 1-20** | **300h** | **60h** |
| overflow + stretch | absorb any slippage from earlier phases. if on schedule: pbr property stubs, better error messages, extra test fixtures | week 21-22 | — | — |
